...
const DB_PATH = process.env.PI_SESSION_USAGE_DB ?? join(homedir(), ".pi", "agent", "session-usage.db");
...
export default function sessionUsageSqliteExtension(pi: ExtensionAPI) {
	...
	pi.registerCommand("usage-stats", {
		description: "Show aggregate token, cost, and session totals from the SQLite tracker",
